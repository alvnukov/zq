...
pub enum Undefined {
    ...
    /// filter with arity
    Filter(Arity),
    /// non-string constant object key (e.g. `{(0):1}` or `{(true):1}`)
    ObjKey(&'static str),
}
...
impl<'s, F> Compiler<&'s str, F> {
    fn invalid_const_obj_key(k: &parse::Term<&'s str>) -> Option<(&'s str, &'static str)> {
        match k {
            parse::Term::Num(n) => Some((*n, "number")),
            parse::Term::Neg(inner) => Self::invalid_const_obj_key(inner),
            parse::Term::Call(name, args) if args.is_empty() && (*name == "true" || *name == "false") => {
                Some((*name, "boolean"))
            }
            parse::Term::Call(name, args) if args.is_empty() && *name == "null" => Some((*name, "null")),
            _ => None,
        }
    }

    fn validate_obj_key(&mut self, k: &parse::Term<&'s str>) {
        if let Some((lit, kind)) = Self::invalid_const_obj_key(k) {
            self.errs.push((lit, Undefined::ObjKey(kind)));
        }
    }
            ...
            BinOp(l, op, r) => {
                ...
                match op {
                    PipeAlt(pats) => {
                        ...
                        let make_branch =
                            |l: parse::Term<&'s str>, pat: parse::Pattern<&'s str>, r: parse::Term<&'s str>| {
                                let mut branch =
                                    parse::Term::BinOp(Box::new(l), Pipe(Some(pat)), Box::new(r));
                                // jq initializes all variables participating in a destructuring
                                // alternation to null so that non-matching branches still have
                                // those names in scope.
                                for v in all_vars.iter().rev() {
                                    branch = parse::Term::BinOp(
                                        Box::new(parse::Term::Call("null", Vec::new())),
                                        Pipe(Some(parse::Pattern::Var(*v))),
                                        Box::new(branch),
                                    );
                                }
                                parse::Term::TryCatch(Box::new(branch), None)
                            };

                        let mut pats = pats.into_iter();
                        let first = pats.next().expect("PipeAlt has at least one pattern");
                        let mut expr = make_branch((*l).clone(), first, (*r).clone());
                        for pat in pats {
                            let next = make_branch((*l).clone(), pat, (*r).clone());
                            expr = parse::Term::BinOp(Box::new(expr), Alt, Box::new(next));
                        }
                        self.term(expr, tr)
                    }
                    _ => {
                        let (l, r) = match op {
                            ...
                            Pipe(ref pat) => {
                                let vars: Vec<_> =
                                    pat.iter().flat_map(|p| p.vars()).copied().collect();
    ...
    fn pattern(&mut self, p: parse::Pattern<&'s str>) -> Pattern<TermId> {
        match p {
            ...
            parse::Pattern::Obj(o) => {
                let iter = o.into_iter().map(|(k, p)| {
                    self.validate_obj_key(&k);
                    (self.iterm(k), self.pattern(p))
                });
                Pattern::Idx(iter.collect())
            }
        }
    ...
    fn call_mod(&mut self, module: &'s str, name: &'s str, args: &[TermId]) -> Term {
        let mut saw_module_alias = false;
        for (mid, module_) in self.imported_mods.iter().rev() {
            if module != *module_ {
                continue;
            }
            saw_module_alias = true;
            if let Some(call) = self.call_mod_id(*mid, name, args) {
                return call;
            }
        }
        if !saw_module_alias {
            return self.fail(module, Undefined::Mod);
        }
    ...
    fn var(&mut self, x: &'s str) -> Term {
        ...
        let current_mid = self.mod_map.len();

        // jq accepts namespaced data-import variables, e.g. `$d::d`,
        // where `import "data" as $d` is also directly accessible as `$d`.
        let namespaced_data_alias = x.split_once("::").and_then(|(alias, scoped)| {
            if !alias.starts_with('$') {
                return None;
            }
            let canonical = alias.strip_prefix('$').unwrap_or(alias);
            (scoped == canonical).then_some(alias)
        });
        ...
        for (x_, mid) in self.imported_vars.iter().rev() {
            let same_name = x == *x_;
            let via_namespace = namespaced_data_alias.is_some_and(|alias| alias == *x_);
            if (same_name || via_namespace) && *mid == current_mid {
                return Term::Var(i);
            } else {
                i += 1;
            }
