...
/// Return cartesian product with left stream as outer loop.
fn cartesian_lr<'a, D: DataT>(l: &'a Id, r: &'a Id, cv: Cv<'a, D>) -> Pairs<'a, ValX<D::V<'a>>> {
    flat_map_with(l.run(cv.clone()), cv, move |l, cv| {
        map_with(r.run(cv), l, |r, l| (l, r))
    })
}

/// Return cartesian product with right stream as outer loop.
///
/// jq evaluates binary operators with this order (observable for `-`, `/`, ...).
fn cartesian_rl<'a, D: DataT>(l: &'a Id, r: &'a Id, cv: Cv<'a, D>) -> Pairs<'a, ValX<D::V<'a>>> {
...
enum DefRunStep<'a, D: DataT, T> {
    Call(Cv<'a, D, T>),
    Out(ValX<T, D::V<'a>>),
}

type DefRunSteps<'a, D, T> = box_iter::BoxIter<'a, DefRunStep<'a, D, T>>;

fn def_run_non_tail<'a, D: DataT, T: 'a>(
    id: &'a Id,
    ...
    let init: DefRunSteps<'a, D, T> = Box::new(cvs.map(|cv| match cv {
        Ok(cv) => DefRunStep::Call(cv),
        Err(e) => DefRunStep::Out(Err(e)),
    }));

    Box::new(crate::Stack::with_item(
        alloc::vec::Vec::from([init]),
        move |step| match step {
            DefRunStep::Call(cv) => {
                let next: DefRunSteps<'a, D, T> = Box::new(run(id, cv).map(DefRunStep::Out));
                ControlFlow::Continue(next)
            }
            DefRunStep::Out(out) => ControlFlow::Break(out),
        },
    ))
}
...
fn def_run<'a, D: DataT, T: 'a>(
    ...
    use core::ops::ControlFlow;
    match tailrec {
        None => def_run_non_tail(id, cvs, run),
...
fn recurse_run<'a, T: Clone + 'a, V: 'a, I: Iterator<Item = ValR<T, V>> + 'a>(
    ...
    enum RecurseStep<T, V> {
        Yield(ValX<T, V>),
        Descend(T),
    }

    type RecurseSteps<'a, T, V> = box_iter::BoxIter<'a, RecurseStep<T, V>>;

    let init: RecurseSteps<'a, T, V> = Box::new(
        core::iter::once(RecurseStep::Yield(Ok(x.clone())))
            .chain(core::iter::once(RecurseStep::Descend(x))),
    );

    Box::new(crate::Stack::with_item(
        alloc::vec::Vec::from([init]),
        move |step| match step {
            RecurseStep::Yield(out) => ControlFlow::Break(out),
            RecurseStep::Descend(node) => {
                let next: RecurseSteps<'a, T, V> = Box::new(f(node).flatten().flat_map(|child| {
                    core::iter::once(RecurseStep::Yield(Ok(child.clone())))
                        .chain(core::iter::once(RecurseStep::Descend(child)))
                }));
                ControlFlow::Continue(next)
            }
        },
...
fn recurse_update<'a, V: ValT + 'a>(v: V, f: BoxUpdate<'a, V>) -> ValXs<'a, V> {
    v.recurse_update(move |v| f.clone()(v))
}

fn cmp_vals<V: ValT>(a: &V, b: &V) -> core::cmp::Ordering {
    a.partial_cmp(b).unwrap_or(core::cmp::Ordering::Equal)
}

fn cmp_opts<V: ValT>(a: &Option<V>, b: &Option<V>) -> core::cmp::Ordering {
...
fn cmp_path_parts<V: ValT>(a: &crate::path::Part<V>, b: &crate::path::Part<V>) -> core::cmp::Ordering {
    use crate::path::Part::{Index, Range};
    use core::cmp::Ordering::{Equal, Greater, Less};
    match (a, b) {
        (Index(a), Index(b)) => cmp_vals(a, b),
        (Index(_), Range(_, _)) => Less,
        (Range(_, _), Index(_)) => Greater,
        (Range(af, at), Range(bf, bt)) => {
            let c = cmp_opts(af, bf);
            if c != Equal {
                c
            } else {
                cmp_opts(at, bt)
...
fn cmp_paths<V: ValT>(a: &crate::path::Path<V>, b: &crate::path::Path<V>) -> core::cmp::Ordering {
    use core::cmp::Ordering::Equal;
    for ((ap, ao), (bp, bo)) in a.0.iter().zip(&b.0) {
        let c = cmp_path_parts(ap, bp);
        if c != Equal {
            return c;
        }
        let c = match (*ao as u8).cmp(&(*bo as u8)) {
            Equal => Equal,
            other => other,
        };
        if c != Equal {
...
fn first_path_part_value<'a, D: DataT>(
    path: &crate::path::Path<Id>,
    cv: Cv<'a, D>,
) -> Option<crate::path::Part<D::V<'a>>> {
    let (part, _opt) = path.0.first()?;

    let eval = |id: &Id, (ctx, v): Cv<'a, D>| -> Option<D::V<'a>> {
        let mut ys = id.run((ctx, v));
        ys.next().and_then(Result::ok)
    };

    use crate::path::Part::{Index, Range};
    match part {
        Index(id) => Some(Index(eval(id, (cv.0, cv.1))?)),
        Range(from, upto) => {
            let from = match from {
                Some(id) => Some(eval(id, (cv.0.clone(), cv.1.clone()))?),
                None => None,
            };
            let upto = match upto {
                Some(id) => Some(eval(id, (cv.0, cv.1))?),
                None => None,
            };
            Some(Range(from, upto))
        }
    }
}

fn first_filter_path_part<'a, D: DataT>(
    id: &Id,
    cv: Cv<'a, D>,
) -> Option<crate::path::Part<D::V<'a>>> {
    match &cv.0.lut().terms[id.0] {
        Ast::Path(_, path) => first_path_part_value(path, cv),
        Ast::Pipe(_, None, r) => first_filter_path_part(r, cv),
        Ast::TryCatch(f, _) => first_filter_path_part(f, cv),
        Ast::Label(f) => first_filter_path_part(f, cv),
        _ => None,
    }
}

fn path_part_error<V: ValT>(part: &crate::path::Part<V>, obj: V) -> Error<V> {
    use crate::path::Part::{Index, Range};
    match part {
        Index(k) => Error::path_expr_access(k.clone(), obj),
        Range(None, None) => Error::path_expr_iter(obj),
        Range(from, upto) => Error::path_expr_access(V::from(from.clone()..upto.clone()), obj),
    }
}

fn rewrite_path_exn_near<V: ValT>(
    exn: Exn<V>,
    part: Option<&crate::path::Part<V>>,
) -> Exn<V> {
    match exn.get_err() {
        Ok(err) => match err.into_path_expr_result() {
            Ok(obj) => Exn::from(match part {
                Some(part) => path_part_error(part, obj),
                None => Error::path_expr_result(obj),
            }),
            Err(err) => Exn::from(err),
        },
        Err(exn) => exn,
    }
}
...
impl<D: DataT> Native<D> {
    ...
    pub const fn new(run: RunPtr<D>) -> Self {
        Self {
            run,
            paths: |cv| box_once(Err(Exn::from(Error::path_expr_result(cv.1 .0)))),
            update: |cv, _| box_once(Err(Exn::from(Error::path_expr_result(cv.1)))),
        }
    }
...
impl Id {
    ...
    pub fn run<'a, D: DataT>(&self, cv: Cv<'a, D>) -> ValXs<'a, D::V<'a>> {
        ...
        match &cv.0.lut().terms[self.0] {
            ...
            Ast::ObjSingle(k, v) => {
                Box::new(cartesian_lr(k, v, cv).map(|(k, v)| Ok(D::V::from_map([(k?, v?)])?)))
            }
            ...
            Ast::Math(l, op, r) => {
                Box::new(cartesian_rl(l, r, cv).map(|(x, y)| Ok(op.run(x?, y?)?)))
            }
            Ast::Cmp(l, op, r) => {
    ...
    pub fn paths<'a, D: DataT>(&self, cv: Cvp<'a, D>) -> ValPathXs<'a, D::V<'a>> {
        let err = |v| box_once(Err(Exn::from(Error::path_expr_result(v))));
        ...
        match &cv.0.lut().terms[self.0] {
            ...
            Ast::Pipe(l, None, r) => {
                let near_part = first_filter_path_part(r, (cv.0.clone(), cv.1 .0.clone()));
                let ys = l
                    .paths((cv.0.clone(), cv.1))
                    .map(move |y| y.map_err(|e| rewrite_path_exn_near(e, near_part.as_ref())));
                flat_map_then_with(ys, cv.0, move |y, ctx| r.paths((ctx, y)))
            }
            ...
            Ast::TryCatch(f, c) => try_catch_run(f.paths((cv.0.clone(), cv.1)), move |e| {
                c.run((cv.0.clone(), e.into_val()))
                    .map(|e| Err(Exn::from(Error::path_expr_result(e?))))
            }),
            Ast::Path(f, path) => {
                let near_part = first_path_part_value(path, (cv.0.clone(), cv.1 .0.clone()));
                let path = path.map_ref(|i| {
                ...
                let ys =
                    f.paths(cv)
                        .map(move |y| y.map_err(|e| rewrite_path_exn_near(e, near_part.as_ref())));
                flat_map_then_with(ys, path, |y, path| {
                    flat_map_then_with(path.explode(), y, |path, y| {
    ...
    pub fn update<'a, D: DataT>(
        ...
        match &cv.0.lut().terms[self.0] {
            ...
            Ast::Path(l, path) => {
                let near_part = first_path_part_value(path, cv.clone());
                let path = path.map_ref(|i| {
                    let cv = cv.clone();
                    crate::into_iter::collect_if_once(move || i.run(cv))
                });
                let f = move |v| {
                    ...
                    for path in path.clone().explode() {
                        match path {
                    ...
                    paths.dedup_by(|a, b| cmp_paths(a, b) == core::cmp::Ordering::Equal);
                    box_once(paths.into_iter().try_fold(v, |acc, path| path.update(acc, &f)))
                };
                let ys = l.update(cv, Box::new(f));
                Box::new(ys.map(move |y| y.map_err(|e| rewrite_path_exn_near(e, near_part.as_ref()))))
            }
            ...
            Ast::CallDef(id, args, skip, _tailrec) => {
                ...
                reduce(cvs, init, move |cv, v| {
                    let ctx = cv.0.clone();
                    let input = v.clone();
                    let ys = id.update((cv.0, v), f.clone());
                    Box::new(ys.map(move |y| match y {
                        Ok(v) => Ok(v),
                        Err(exn) => match exn.get_err() {
                            Ok(err) => match err.into_path_expr_result() {
                                Ok(_obj) => {
                                    let mut run = id.run((ctx.clone(), input.clone()));
                                    match run.next() {
                                        Some(Ok(out)) => {
                                            Err(Exn::from(Error::path_expr_result(out)))
                                        }
                                        Some(Err(e)) => Err(e),
                                        None => Err(Exn::from(Error::path_expr_result(
                                            input.clone(),
                                        ))),
                                    }
                                }
                                Err(err) => Err(Exn::from(err)),
                            },
                            Err(exn) => Err(exn),
                        },
                    }))
