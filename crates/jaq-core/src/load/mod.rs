...
pub struct Module<S, B = Vec<Def<S>>> {
    ...
    /// explicit include/import declarations
    pub(crate) deps: Vec<Dep<S>>,
...
/// Include/import dependency declaration as written in source.
#[derive(Clone)]
pub struct Dep<S> {
    path: S,
    as_: Option<S>,
...
impl<S: core::ops::Deref<Target = str> + Clone, B> parse::Module<S, B> {
    fn map(
        self,
        ...
        for (path, as_, meta) in self.deps {
            let is_data = as_.as_ref().is_some_and(|x| x.starts_with('$'));
            deps.push(Dep {
                path: path.clone(),
                as_: as_.clone(),
                meta: meta.clone(),
...
impl<S, B> Module<S, B> {
    /// Module metadata term.
    pub fn meta(&self) -> Option<&Term<S>> {
        self.meta.as_ref()
    }

    /// Include/import declarations of this module.
    pub fn deps(&self) -> &[Dep<S>] {
        &self.deps
    }
...
impl<S> Dep<S> {
    /// Path used in `include`/`import`.
    pub fn path(&self) -> &S {
        &self.path
    }

    /// Optional alias (`as ...`).
    pub fn as_(&self) -> Option<&S> {
        self.as_.as_ref()
    }
    ...
    /// True for data imports (`import "x" as $var`).
...
impl<'a> Import<'a, &'a str, PathBuf> {
    ...
    pub fn find(self, paths: &[PathBuf], ext: &str) -> Result<PathBuf, String> {
        ...
        let rel = Path::new(self.path).to_path_buf();
        if !rel.is_relative() {
            Err("non-relative path")?
        }
        let rel_with_ext = rel.with_extension(ext);
        let rel_dir_fallback = rel.file_name().map(|name| {
            let mut p = rel.clone();
            p.push(name);
            p.set_extension(ext);
            p
        ...
        meta.chain(paths.iter().map(expand))
            .flat_map(|path| {
                let with_ext = path.join(&rel_with_ext);
                let in_dir = rel_dir_fallback.as_ref().map(|rel| path.join(rel));
                core::iter::once(with_ext).chain(in_dir)
            })
