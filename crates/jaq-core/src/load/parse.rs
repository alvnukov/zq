...
pub enum Expect<S> {
    ...
    /// `0;`
    Nothing,
    /// custom parse diagnostic message
    Custom(S),
}
...
pub enum BinaryOp<S> {
    ...
    /// Application with jq destructuring alternation, i.e. `l as p1 ?// p2 | r`.
    PipeAlt(Vec<Pattern<S>>),
    ...
    fn climb(self, tail: &mut impl Iterator<Item = (BinaryOp<S>, Self)>) -> Self {
        let tail = core::iter::from_fn(|| {
            tail.next().map(|(op, tm)| match op {
                BinaryOp::Pipe(Some(_)) | BinaryOp::PipeAlt(_) => (op, tm.climb(tail)),
                _ => (op, tm),
            })
        });
...
impl<'s, 't> Parser<'s, 't> {
    fn const_string_parts(parts: &[StrPart<&'s str, Term<&'s str>>]) -> bool {
        parts.iter().all(|p| !matches!(p, StrPart::Term(_)))
    }

    fn const_string_term(t: &Term<&'s str>) -> bool {
        matches!(t, Term::Str(None, parts) if Self::const_string_parts(parts))
    }

    fn const_term(t: &Term<&'s str>) -> bool {
        match t {
            Term::Call("null", args) | Term::Call("true", args) | Term::Call("false", args) => {
                args.is_empty()
            }
            Term::Num(_) => true,
            Term::Neg(inner) => matches!(&**inner, Term::Num(_)),
            Term::Str(None, parts) => Self::const_string_parts(parts),
            Term::Arr(None) => true,
            Term::Arr(Some(items)) => {
                let mut stack = Vec::from([items.as_ref()]);
                while let Some(cur) = stack.pop() {
                    if let Term::BinOp(l, BinaryOp::Comma, r) = cur {
                        stack.push(l);
                        stack.push(r);
                    } else if !Self::const_term(cur) {
                        return false;
                    }
                }
                true
            }
            Term::Obj(entries) => entries.iter().all(|(k, v)| {
                Self::const_string_term(k)
                    && v.as_ref().is_none_or(|v| Self::const_term(v))
            }),
            _ => false,
        }
    }
    ...
    fn op(&mut self, with_comma: bool) -> Result<'s, 't, Option<BinaryOp<&'s str>>> {
        ...
        self.try_maybe(|p| match p.i.next() {
            Some(Token(s, _)) => Ok(Some(match *s {
                "|" => BinaryOp::Pipe(None),
                "as" => {
                    let mut xs = Vec::from([p.pattern()?]);
                    loop {
                        let i = p.i.clone();
                        match (p.i.next(), p.i.next()) {
                            (Some(Token("?", _)), Some(Token("//", _))) => xs.push(p.pattern()?),
                            _ => {
                                p.i = i;
                                break;
                            }
                    ...
                    p.just("|")?;
                    if xs.len() == 1 {
                        BinaryOp::Pipe(Some(xs.pop().unwrap()))
                    } else {
                        BinaryOp::PipeAlt(xs)
                    }
                }
    ...
    /// Parse a binary operator that is valid in jq's `Expr` grammar.
    ///
    /// Notably, this excludes `|`, `,`, and `as ... |`.
    fn op_expr(&mut self) -> Result<'s, 't, Option<BinaryOp<&'s str>>> {
        use ops::{Cmp, Math};
        self.try_maybe(|p| match p.i.next() {
            Some(Token(s, _)) => Ok(Some(match *s {
                "+" => BinaryOp::Math(Math::Add),
    ...
    fn pattern(&mut self) -> Result<'s, 't, Pattern<&'s str>> {
        match self.i.next() {
            ...
            next @ Some(Token(full, Tok::Block(tokens))) => match &full[..1] {
                ...
                "{" => {
                    let entries = self.with(tokens, "", |p| p.obj_items(Self::pat_obj_entry));
                    let entries = entries.into_iter().flatten().collect();
                    Ok(Pattern::Obj(entries))
                }
    ...
    /// Parse jq's `Expr` grammar, i.e. expressions without `|`, `,`, and `as`.
    fn expr(&mut self) -> Result<'s, 't, Term<&'s str>> {
        let head = self.atom()?;
        let mut tail = Vec::new();
        while let Some(op) = self.op_expr()? {
            tail.push((op, self.atom()?));
        }
        Ok(head.climb(&mut tail.into_iter()))
    }
    ...
    fn atom(&mut self) -> Result<'s, 't, Term<&'s str>> {
        let tm = match self.i.next() {
            ...
            Some(Token(fold @ ("reduce" | "foreach"), _)) => {
                // jq grammar allows a full Expr here, not just an atomic term.
                let xs = self.expr()?;
                self.just("as")?;
                let x = self.pattern()?;
    ...
    /// * ` a : pat`
    /// * `"a": pat`
    fn pat_obj_entry(&mut self) -> Result<'s, 't, Vec<(Term<&'s str>, Pattern<&'s str>)>> {
        let i = self.i.clone();
        let key = match self.i.next() {
            Some(Token(x, Tok::Var)) => {
                if matches!(self.i.as_slice(), [Token(":", _), ..]) {
                    self.just(":")?;
                    let pat = self.pattern()?;
                    // jq's `$k: pat` binds `$k` to the matched field value
                    // and then applies `pat` to that same value.
                    return Ok(Vec::from([
                        (Term::from_str(&x[1..]), Pattern::Var(*x)),
                        (Term::from_str(&x[1..]), pat),
                    ]));
                } else {
                    return Ok(Vec::from([(Term::from_str(&x[1..]), Pattern::Var(*x))]));
                }
            }
            Some(Token(full, Tok::Block(tokens))) if full.starts_with('(') => {
                self.with(tokens, ")", Self::term)
    ...
    fn path(&mut self) -> Result<'s, 't, Path<Term<&'s str>>> {
        ...
        while let Some(key) = self.dot() {
            if key.is_empty() {
                // jq permits `.<path>` spellings such as `.a.[]`.
                // If `.` is immediately followed by a bracket path-part,
                // treat it as a separator only.
                let next_is_bracket = matches!(
                    self.i.as_slice(),
                    [Token(full, Tok::Block(_)), ..] if full.starts_with('[')
                );
                if !next_is_bracket {
                    let key = self.str_key()?;
                    path.push((path::Part::Index(key), self.opt()));
                }
            } else {
                path.push((path::Part::Index(Term::from_str(key)), self.opt()));
            }
    ...
    fn bare_str(&mut self) -> Result<'s, 't, &'s str> {
        match self.i.next() {
            next @ Some(Token(_, Tok::Str(parts))) => match parts[..] {
                [StrPart::Str(s)] => Ok(s),
                _ => Err((Expect::Custom("Import path must be constant"), next)),
            },
            next => Err((Expect::Str, next)),
        }
    }
    ...
    fn dep(&mut self, import: bool) -> Result<'s, 't, Dep<&'s str>> {
        ...
        let meta_start = self.i.as_slice().first();
        let meta = !matches!(self.i.as_slice(), [Token(";", _), ..]);
        let meta = meta.then(|| self.term()).transpose()?;
        if let Some(meta) = &meta {
            if !Self::const_term(meta) {
                return Err((Expect::Custom("Module metadata must be constant"), meta_start));
            }
            if !matches!(meta, Term::Obj(_)) {
                return Err((Expect::Custom("Module metadata must be an object"), meta_start));
            }
        }
        Ok((path, name, meta))
    }
    ...
    {
        let mut meta_start: Option<&'t Token<&'s str>> = None;
        let meta = if matches!(self.i.as_slice().first(), Some(Token("module", _))) {
            self.i.next();
            meta_start = self.i.as_slice().first();
            Some(self.terminated(Self::term)?)
        } else {
            None
        };
        if let Some(meta) = &meta {
...
impl<S> prec_climb::Op for BinaryOp<S> {
    fn precedence(&self) -> usize {
        ...
        match self {
            Self::Pipe(None) => 0,
            Self::Comma => 1,
            Self::Pipe(Some(_)) | Self::PipeAlt(_) => 2,
