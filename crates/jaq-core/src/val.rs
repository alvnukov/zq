    ...
    + Neg<Output = ValR<Self>>
{
    ...
    /// Apply recursive update semantics used by `recurse |= f`.
    ///
    /// The default implementation is recursive and may be overridden by value
    /// implementations that need a stack-safe strategy.
    fn recurse_update<'a, F>(self, f: F) -> ValXs<'a, Self>
    where
        Self: 'a,
        F: Fn(Self) -> ValXs<'a, Self> + Clone + 'a,
    {
        use crate::path::Opt::Optional;
        let f_children = f.clone();
        crate::box_iter::then(
            self.map_values(Optional, move |v| v.recurse_update(f_children.clone())),
            move |v| f(v),
        )
    }
