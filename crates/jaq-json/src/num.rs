...
const MAX_SAFE_INT_F64: i128 = 9_007_199_254_740_992; // 2^53
...
impl Num {
    ...
    pub(crate) fn as_f64(&self) -> f64 {
        match self {
            Self::Int(n) => *n as f64,
            Self::BigInt(n) => n.to_f64().unwrap_or_else(|| {
    ...
    fn int_arith_should_use_float(&self) -> bool {
        match self {
            Self::Int(i) => (*i as i128).abs() > MAX_SAFE_INT_F64,
            Self::BigInt(i) => {
                let mag = i.magnitude();
                match mag.bits() {
                    0..=53 => false,
                    54 => mag.to_u64().map_or(true, |v| v > (1u64 << 53)),
                    _ => true,
                }
            }
            _ => false,
    ...
    fn from_f64_arith(f: f64) -> Self {
        if f.is_finite() && f.fract() == 0.0 {
            // Arithmetic on large integers in jq follows double rounding,
            // but still renders as an integer-looking literal.
            return Self::Dec(alloc::format!("{f:.0}").into());
        }
        Self::Float(f)
    }
...
impl core::ops::Add for Num {
    ...
    fn add(self, rhs: Self) -> Self::Output {
        ...
        if self.is_int()
            && rhs.is_int()
            && (self.int_arith_should_use_float() || rhs.int_arith_should_use_float())
        {
            return Self::from_f64_arith(self.as_f64() + rhs.as_f64());
        }
        match (self, rhs) {
