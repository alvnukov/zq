...
impl Error {
    ...
    pub(crate) fn kind(&self) -> &hifijson::Error {
        &self.1
    }
}
...
pub fn parse_many(slice: &[u8]) -> impl Iterator<Item = Result<Val, Error>> + '_ {
    ...
    core::iter::from_fn(move || {
        Some(
            parse_entry(ws_tk(&mut lexer)?, &mut lexer)
                .map_err(|e| Error(offset(lexer.as_slice()), e)),
        )
    })
}
...
pub fn read_many<'a>(read: impl io::BufRead + 'a) -> impl Iterator<Item = io::Result<Val>> + 'a {
    ...
    core::iter::from_fn(move || {
        let v = ws_tk(&mut lexer).map(|next| parse_entry(next, &mut lexer).map_err(invalid_data));
...
fn parse<L: LexAlloc>(next: u8, lexer: &mut L) -> Result<Val, hifijson::Error> {
    parse_with_depth(next, lexer, 0)
}

const MAX_PARSE_DEPTH: usize = 10000;

#[cfg(feature = "std")]
fn parse_entry<L: LexAlloc>(next: u8, lexer: &mut L) -> Result<Val, hifijson::Error> {
    // jq-compat depth tests use nesting close to 10k.
    stacker::maybe_grow(32 * 1024 * 1024, 64 * 1024 * 1024, || parse(next, lexer))
}
...
fn parse_with_depth<L: LexAlloc>(
    next: u8,
    lexer: &mut L,
    depth: usize,
) -> Result<Val, hifijson::Error> {
    if depth >= MAX_PARSE_DEPTH {
        Err(hifijson::Error::Depth)?;
    }
    ...
    Ok(match next {
        ...
        b'[' => Val::Arr(Rc::new({
            ...
            lexer.discarded().seq(b']', ws_tk, |next, lexer| {
                arr.push(parse_with_depth(next, lexer, depth + 1)?);
                Ok::<_, hifijson::Error>(())
            })?;
            arr.into()
        })),
        b'{' => Val::obj({
            ...
            lexer.discarded().seq(b'}', ws_tk, |next, lexer| {
                let key = parse_with_depth(next, lexer, depth + 1)?;
                lexer.expect(ws_tk, b':').ok_or(Expect::Colon)?;
                let value = parse_with_depth(ws_tk(lexer).ok_or(Expect::Value)?, lexer, depth + 1)?;
