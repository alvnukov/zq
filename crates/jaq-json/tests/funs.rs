...
#[test]
fn jq_string_mul_edge_cases() {
    give(
        json!([-1.0, -0.5, 0.0, 0.5, 1.0, 1.5, 3.7, 10.0]),
        r#"[.[] * "abc"]"#,
        json!([
            null,
            null,
    ...
    give(
        json!("abc"),
        r#"[. * ("NaN" | fromjson), . * ("-NaN" | fromjson)]"#,
        json!([null, null]),
    );
}

#[test]
fn jq_large_integer_arithmetic_contract() {
    give(
        json!(13911860366432393_i64),
        ". - 10",
        json!(13911860366432382_i64),
    );
    ...
    gives(
        json!([
            ...
            9007199254740992_i64,
            9007199254740993_i64,
            13911860366432393_i64
        ]),
        ".[] as $n | $n+0 | [., tostring, . == $n]",
        [
            json!([-9007199254740992_i64, "-9007199254740992", true]),
            json!([-9007199254740992_i64, "-9007199254740992", true]),
            json!([9007199254740992_i64, "9007199254740992", true]),
            json!([9007199254740992_i64, "9007199254740992", true]),
            json!([13911860366432392_i64, "13911860366432392", true]),
        ],
    );
}

#[test]
fn jq_dec_length_preserves_exponent_form() {
    give(
        json!(null),
        "[1E+1000,-1E+1000 | length | tojson] | unique",
        json!(["1E+1000"]),
    );
}
