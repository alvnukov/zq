...
/// Convert an array of Unicode codepoints into a UTF-8 string.
/// Invalid codepoints are replaced with U+FFFD, matching jq behavior.
fn implode<V: ValT>(xs: &[V]) -> Result<Vec<u8>, Error<V>> {
    fn as_numeric_codepoint<V: ValT>(x: &V) -> Result<isize, Error<V>> {
        if let Some(i) = x.as_isize() {
            return Ok(i);
        }
        ...
        Err(Error::str(format_args!(
            "{ty} ({x}) can't be imploded, unicode codepoint needs to be numeric"
        )))
    }
    ...
    for x in xs {
        ...
        let i = as_numeric_codepoint(x)?;
        let cp = match u32::try_from(i) {
            Ok(cp) if cp <= 0x10_FFFF && !(0xD800..=0xDFFF).contains(&cp) => cp,
            _ => 0xFFFD,
        };
...
#[cfg(feature = "format")]
fn decode_uri_percent_strict(input: &str) -> Option<Vec<u8>> {
    ...
    Box::new([
        ...
        ("decode_uri", v(0), |cv| {
            bome(cv.1.try_as_utf8_bytes().and_then(|s| {
                let input = core::str::from_utf8(s).map_err(Error::str)?;
                let decoded =
                    decode_uri_percent_strict(input).ok_or_else(|| invalid_uri_encoding(input))?;
                if core::str::from_utf8(&decoded).is_err() {
                    return Err(invalid_uri_encoding(input));
                }
                Ok(ValT::from_utf8_bytes(decoded))
            }))
        }),
        ...
        ("decode_base64", v(0), |cv| {
            ...
            bome(cv.1.try_as_utf8_bytes().and_then(|s| {
                let input = core::str::from_utf8(s).map_err(Error::str)?;
                if input.as_bytes().iter().all(|b| *b == b'=') {
                    return Ok(ValT::from_utf8_bytes([]));
                }

                let decoded = STANDARD.decode(input).or_else(|e| match e {
                    DecodeError::InvalidPadding | DecodeError::InvalidLength(_) => {
                ...
                match decoded {
                    Ok(bytes) => Ok(ValT::from_utf8_bytes(bytes)),
                    Err(DecodeError::InvalidLength(_) | DecodeError::InvalidLastSymbol(_, _)) => {
                        Err(trailing_base64_byte(input))
                    }
                    Err(DecodeError::InvalidByte(_, _) | DecodeError::InvalidPadding) => {
                        Err(invalid_base64_data(input))
                    }
                }
            }))
        }),
    ...
    Box::new([
        ...
        ("strftime", v(1), |cv| {
            unary(cv, |v, fmt| {
                let fmt = fmt
                    .as_utf8_bytes()
                    .ok_or_else(|| Error::str("strftime/1 requires a string format"))
                    .and_then(|s| core::str::from_utf8(s).map_err(Error::str))?;
                time::strftime("strftime/1", &v, fmt, TimeZone::UTC)
            })
        }),
        ("strflocaltime", v(1), |cv| {
            unary(cv, |v, fmt| {
                let fmt = fmt
                    .as_utf8_bytes()
                    .ok_or_else(|| Error::str("strflocaltime/1 requires a string format"))
                    .and_then(|s| core::str::from_utf8(s).map_err(Error::str))?;
                time::strftime("strflocaltime/1", &v, fmt, TimeZone::system())
            })
        }),
        ("gmtime", v(0), |cv| {
