...
struct ImportMetadata {
    raw_object: Option<IndexMap<String, ZqValue>>,
...
impl Parser {
    ...
    fn parse_module_directive(&mut self) -> Result<(), String> {
        match self.peek() {
            Token::ModuleKw => {
                self.bump();
            ...
            Token::ImportKw => {
                ...
                let alias = match self.peek() {
                    Token::Ident(name) => {
                        let name = name.clone();
                        self.bump();
                        ImportAlias::Namespace(name)
                    }
                    Token::Binding(name) => {
                        let name = name.clone();
                        self.bump();
                        ImportAlias::Binding(name)
                    }
                    other => {
                        return Err(format!(
                ...
                match alias {
                    ImportAlias::Namespace(namespace) => {
                        ...
                        if self.resolve_imports {
                            match self.load_module_query(&module, metadata.search_dirs.as_deref()) {
                                Ok(query) => {
                                    self.import_module_functions(query.functions, Some(&namespace))?
                                }
                                Err(err)
                    ...
                    ImportAlias::Binding(binding) => {
                        ...
                        if self.resolve_imports {
                            match self.load_module_data(
                                &module,
                                metadata.search_dirs.as_deref(),
                                metadata.raw,
                            ) {
                                Ok(value) => {
                                    self.register_imported_binding(binding.clone(), value.clone());
                                    self.register_imported_binding(
                                        format!("{binding}::{binding}"),
                                        value,
                                    );
                                }
                                Err(_err) if metadata.optional => {}
                                Err(err) => return Err(err),
                            }
                        }
                    }
            ...
            Token::IncludeKw => {
                ...
                if self.resolve_imports {
                    match self.load_module_query(&module, metadata.search_dirs.as_deref()) {
                        Ok(query) => self.import_module_functions(query.functions, None)?,
                        Err(err) if metadata.optional && err.starts_with("module not found:") => {}
                        Err(err) => return Err(err),
                    }
                }
                Ok(())
    ...
    fn module_descriptor_value(&self) -> ZqValue {
    ...
    fn is_canonical_while_def(&self, function_id: usize) -> bool {
    ...
    fn is_canonical_until_def(&self, function_id: usize) -> bool {
        let Some(function) = self.function_by_id(function_id) else {
            return false;
        };
...
fn module_path_literal(stage: &Stage) -> Result<String, String> {
...
fn ensure_const_object_metadata(stage: &Stage) -> Result<(), String> {
    match const_stage_value(stage) {
...
// `search`, `optional`, `raw`.
fn import_metadata_from_object(
...
fn jq_origin_dir() -> Option<PathBuf> {
    std::env::current_exe()
        .ok()
        .and_then(|path| path.parent().map(Path::to_path_buf))
}

fn home_dir_like_jq() -> Option<PathBuf> {
...
fn canonicalize_module_candidate(path: PathBuf) -> PathBuf {
    fs::canonicalize(&path).unwrap_or(path)
}

fn normalize_search_root_like_jq(
    root: &Path,
    current_module_dir: Option<&Path>,
    jq_origin: Option<&Path>,
) -> PathBuf {
...
fn const_stage_value(stage: &Stage) -> Option<ZqValue> {
    match stage {
...
fn validate_module_relpath(path: &str) -> Result<(), String> {
    if path.contains('\\') {
        return Err(format!(
...
fn module_code_candidates(root: &Path, module: &str) -> Vec<PathBuf> {
...
fn module_data_candidates(root: &Path, module: &str) -> Vec<PathBuf> {
    let mut out = Vec::with_capacity(3);
...
fn parse_number_literal(raw: &str) -> Result<ZqValue, String> {
...
fn fold_large_integer_literal_equality(lhs: &Stage, rhs: &Stage, op: BinaryOp) -> Option<Stage> {
...
fn special_number_literal(raw: &str) -> ZqValue {
...
fn normalize_jq_float_text(raw: &str) -> Option<String> {
    if raw.ends_with('.') {
...
fn const_object_key_error(value: &ZqValue) -> Option<String> {
...
fn zq_type_name(value: &ZqValue) -> &'static str {
    match value {
        ZqValue::Null => "null",
