...
fn run_builtin(filter: Builtin, input: ZqValue) -> Result<ZqValue, String> {
    match filter {
...
fn run_env(_input: ZqValue) -> Result<ZqValue, String> {
...
fn run_length(input: ZqValue) -> Result<ZqValue, String> {
    match input {
...
fn run_math_binary(op: MathBinaryOp, lhs: ZqValue, rhs: ZqValue) -> Result<ZqValue, String> {
...
fn run_join(input: ZqValue, separator: ZqValue) -> Result<ZqValue, String> {
    let values = iter_values_like_jq(input)?;
...
#[cfg(unix)]
fn run_strptime(input: ZqValue, format: ZqValue) -> Result<ZqValue, String> {
...
#[cfg(not(unix))]
fn run_strptime(_input: ZqValue, _format: ZqValue) -> Result<ZqValue, String> {
    Err("strptime/1 not implemented on this platform".to_string())
}

// Ported from jq/src/builtin.c:f_strftime() and f_strflocaltime().
fn run_strftime(input: ZqValue, format: ZqValue, local: bool) -> Result<ZqValue, String> {
